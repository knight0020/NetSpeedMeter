...
ICON_SIZE = 64               # tray icon image size (px)
...
class NetworkMonitor:
    ...
    real-time upload/download speed. Runs in its own thread and pushes
    updates onto a thread-safe queue consumed by the UI thread.
    """
    ...
    def _run(self):
        while self._running:
            ...
            # Keep queue small - drop old snapshot if UI hasn't consumed it yet
            try:
                while self.update_queue.qsize() > 2:
...
class TrayIconManager:
    ...
    def _load_font(self):
        # Try a few common fonts; fall back to PIL default bitmap font.
        candidates = [
            "arial.ttf", "Arial.ttf",
        ...
        for c in candidates:
            try:
                return ImageFont.truetype(c, 22)
            except Exception:
                continue
    ...
    def _make_icon_image(self, down_speed: float) -> "Image.Image":
        ...
        # Compact label, e.g. "1.2M" or "340K" or "0"
        if down_speed >= 1024 * 1024:
            label = f"{down_speed / (1024*1024):.1f}M"
        ...
        try:
            ...
            w, h = draw.textsize(label, font=self._font)
            x = (ICON_SIZE - w) / 2
            y = (ICON_SIZE - h) / 2

        draw.text((x, y), label, fill=(255, 255, 255, 255), font=self._font)
        return img
...
class FloatingOverlay:
    ...
    def _poll_updates(self):
        ...
        try:
            while True:
                latest = self.monitor.update_queue.get_nowait()
        except queue.Empty:
            pass
...
class SpeedMeterWindow:
    ...
    # ---- Update loop --------------------------------------------------------
    def _poll_updates(self):
        """Drains the monitor's update queue and refreshes the UI. Runs on the Tk main loop."""
        latest = None
        try:
            while True:
                latest = self.monitor.update_queue.get_nowait()
